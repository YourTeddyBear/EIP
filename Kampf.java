...
public class Kampf {
    ...
    public Einheit findeNaechstesZiel(int angreiferPosition) {
        if (this.einheiten[angreiferPosition] instanceof Schaf||this.einheiten[angreiferPosition].equals(null) ) {
            return null;
        } else {
            while(true){
    ...
    // simuliert den gesamten Kampf und endet erst, wenn eine Fraktion gewonnen
    // hat.
    // Die Rückgabe ist die siegreiche Einheit, die den letzten Schlag vollführt
    // hat.
    public Einheit kaempfen() {
        ...
        while (true) {
            ...
            if (angreifer instanceof Krieger && angreifer.lebtNoch()) {
                ...
                if (ziel == angreifer) // in diesem Fall wurde kein gueltiges
                                       // Ziel gefunden => Kampfende
                    return angreifer;
                if (ziel != null)
